...
from .hooks import CommandContext, CommandRejected, get_hook_provider
...
class ErrorQueue:
    ...
    def append_message(self, message: bytes) -> None:
...
class Device(Component):
    ...
    def enqueue_error(self, error: CommandRejected) -> None:
        if error.error_key in self._error_map:
            self._error_map[error.error_key].set(error.error_key)
        escaped = error.message.replace('"', "'")
        message = to_bytes(f'{error.code},"{escaped}"')
        for queue in self._error_queues.values():
            queue.append_message(message)

    def clear_errors(self) -> None:
        for register in self._status_registers.values():
            register.clear()
        for queue in self._error_queues.values():
            queue.clear()
    ...
    def _match(self, query: bytes) -> Optional[OptionalBytes]:
        ...
        provider = get_hook_provider()
        if provider is None:
            return self._match_native(query)

        context = CommandContext(self, self.resource_name or "", query)
        try:
            provider.before_command(context)
            self._validate_native_setter(query)
        except CommandRejected as exc:
            self.enqueue_error(exc)
            provider.on_error(context, exc)
            raise

        response = self._match_native(query)
        if response is not None:
            response = provider.dynamic_response(context, response)
            provider.after_command(context, response)
        return response

    def _validate_native_setter(self, query: bytes) -> None:
        decoded = query.decode("utf-8")
        for name, parser, _, _ in self._setters:
            try:
                value = parser(decoded)
            except ValueError:
                continue
            try:
                self._properties[name].validate_value(value)
            except ValueError as exc:
                raise CommandRejected(-222, "Data out of range") from exc
            return

    def _match_native(self, query: bytes) -> Optional[OptionalBytes]:
        """Match using the unmodified PyVISA-sim component machinery."""
